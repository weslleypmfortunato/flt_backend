...
addOrdersRouter.post('/orders/new', isAuthenticatedMiddleware, async (req, res) => {
  const {workOrderNumber, productName, productDescription, orderQty, priority, owner, status, remarks} = req.body

  try {

    const newOrder = await Orders.create({ workOrderNumber, productName, productDescription, orderQty, priority, owner, status, remarks })
